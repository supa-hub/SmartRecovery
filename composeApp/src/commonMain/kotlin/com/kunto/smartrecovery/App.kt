    ...
    MyApplicationTheme {
        ...
        NavHost(
            ...
            composable<ChooseBLEDevicesPopUp> {backStackEntry ->
                ...
                ChooseDevicesDialog(viewModel { ChooseBLEDevicesViewModel { currSession.updateValues(it) } }, navController, chooseBLEDevicesPopUp.sessionName)
            }
            composable<RequestPermissions> { backStackEntry ->
                ...
                when (Route.entries.getOrElse(requestPermissions.destination) { Route.Main }) {
                    Route.Main -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, Main)
                    Route.AllSavedSessions -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, AllSavedSessions)
                    Route.ChooseBLEDevicesPopUp -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, ChooseBLEDevicesPopUp())
                    Route.CurrentSession -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, CurrentSession)
                    Route.NewSessionName -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, NewSessionName)
                    Route.UserProfileGetter -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, UserProfileGetter)
                    Route.RequestPermissions -> PermissionPage(navController, viewModel { PermissionsViewModel(controller) }, Main)
                }
            }
        }
    }
